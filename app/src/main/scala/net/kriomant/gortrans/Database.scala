...
import net.kriomant.gortrans.utils.{BooleanUtils, closing}
...
object Database {
  ...
  object RoutesTable {
    ...
    val ALL_COLUMNS: Array[String] = Array(
    ...
    class Cursor(cursor: android.database.Cursor) extends CursorWrapper(cursor) {
      def id: Long = cursor.getLong(ID_COLUMN_INDEX)

      def vehicleType: core.VehicleType.Value = VehicleType(cursor.getInt(VEHICLE_TYPE_COLUMN_INDEX))

      def externalId: String = cursor.getString(EXTERNAL_ID_COLUMN_INDEX)

      def name: String = cursor.getString(NAME_COLUMN_INDEX)

      def firstStopName: String = cursor.getString(FIRST_STOP_NAME_COLUMN_INDEX)

      def lastStopName: String = cursor.getString(LAST_STOP_NAME_COLUMN_INDEX)
    }
  ...
  object RoutePointsTable {
    ...
    class Cursor(cursor: android.database.Cursor) extends CursorWrapper(cursor) {
      def routeId: Long = cursor.getLong(ROUTE_ID_COLUMN_INDEX)

      def index: Int = cursor.getInt(INDEX_COLUMN_INDEX)

      def latitude: Float = cursor.getFloat(LATITUDE_COLUMN_INDEX)

      def longitude: Float = cursor.getFloat(LONGITUDE_COLUMN_INDEX)
    }
  ...
  object SchedulesTable {
    ...
    class Cursor(cursor: android.database.Cursor) extends CursorWrapper(cursor) {
      ...
      def scheduleType: core.ScheduleType.Value = ScheduleType(cursor.getInt(SCHEDULE_TYPE_COLUMN_INDEX))

      def scheduleName: String = cursor.getString(SCHEDULE_NAME_COLUMN_INDEX)

      def direction: core.Direction.Value = Direction(cursor.getInt(DIRECTION_COLUMN_INDEX))

      def stopId: Int = cursor.getInt(STOP_ID_COLUMN_INDEX)

      def schedule: Seq[(Int, Int)] = cursor.getString(SCHEDULE_COLUMN_INDEX) match {
        case "" => Seq()
  ...
  object ScheduleStopsTable {
    ...
    val ALL_COLUMNS: Array[String] = Array(ID_COLUMN, NAME_COLUMN, STOP_ID_COLUMN)
  ...
  object RouteGroupsTable {
    ...
    val ALL_COLUMNS: Array[String] = Array(ID_COLUMN, NAME_COLUMN)
  ...
  object RouteGroupItemsTable {
    ...
    val ALL_COLUMNS: Array[String] = Array(ID_COLUMN, GROUP_ID_COLUMN, ROUTE_ID_COLUMN)
    ...
    class Cursor(cursor: android.database.Cursor) extends CursorWrapper(cursor) {
      ...
      def groupId: Long = cursor.getLong(GROUP_ID_COLUMN_INDEX)
  ...
  object RouteGroupList {
    ...
    val ALL_COLUMNS: Array[String] = Array(GROUP_ID_COLUMN, GROUP_NAME_COLUMN, VEHICLE_TYPE_COLUMN, ROUTE_NAME_COLUMN)
    ...
    class Cursor(cursor: android.database.Cursor) extends CursorWrapper(cursor) {
      ...
      def groupName: String = cursor.getString(GROUP_NAME_COLUMN_INDEX)

      def vehicleType: core.VehicleType.Value = VehicleType(cursor.getInt(VEHICLE_TYPE_COLUMN_INDEX))

      def routeName: String = cursor.getString(ROUTE_NAME_COLUMN_INDEX)
    }
  ...
  object NewsTable {
    ...
    class Cursor(cursor: android.database.Cursor) extends CursorWrapper(cursor) {
      ...
      def title: String = cursor.getString(TITLE_COLUMN_INDEX)

      def content: String = cursor.getString(CONTENT_COLUMN_INDEX)

      def readMoreLink: Option[String] = !cursor.isNull(READ_MORE_LINK_COLUMN_INDEX) ? cursor.getString(READ_MORE_LINK_COLUMN_INDEX)
